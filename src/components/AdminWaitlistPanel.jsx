          ...
          <button
            ...
            className={`px-4 py-1.5 text-sm font-medium border transition-colors ${
              filter === f
                ...
                : 'bg-white text-neutral-600 border-neutral-300 hover:border-[#0CCE6B]'
            }`}
          >
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map(entry => (
            <div
              key={entry.id}
              className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-4 bg-neutral-50 border border-neutral-200"
            >
              {/* Info */}
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-neutral-600 truncate">{entry.email}</p>
                {entry.interest && (
                  <p className="text-sm text-neutral-500 italic">"{entry.interest}"</p>
                )}
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <span
                  className={`inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium border ${STATUS_STYLES[entry.status]}`}
                >
                ...
                {entry.status !== 'approved' && (
                  <button
                    ...
                    disabled={!!updating}
                    className="flex items-center gap-1 px-3 py-1.5 bg-[#0CCE6B] hover:bg-[#0BBE60] text-white text-xs font-medium transition-colors disabled:opacity-50"
                ...
                {entry.status !== 'denied' && (
                  <button
                    ...
                    disabled={!!updating}
                    className="flex items-center gap-1 px-3 py-1.5 bg-red-100 hover:bg-red-200 text-red-600 text-xs font-medium transition-colors disabled:opacity-50"
