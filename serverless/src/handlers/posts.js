  ...
  try {
    ...
    if (mediaIds.length > 0) {
      ...
      for (const media of mediaRecords) {
        ...
        // Check if media is video/audio/pdf by type OR file extension
        ...
        const isPdfByExtension = media.s3Key && /\.pdf$/i.test(media.s3Key);
        const needsSignedUrl = media.type === 'video' || media.type === 'audio' || media.type === 'pdf' || isVideoByExtension || isAudioByExtension || isPdfByExtension;

        // Generate signed URL for the main media file (for videos, audio, and PDFs)
  ...
  try {
    ...
    if (post.mediaId) {
      ...
      if (mediaRecord) {
        ...
        const isPdfByExtension = mediaRecord.s3Key && /\.pdf$/i.test(mediaRecord.s3Key);
        const needsSignedUrl = mediaRecord.type === 'video' || mediaRecord.type === 'audio' || mediaRecord.type === 'pdf' || isVideoByExtension || isAudioByExtension || isPdfByExtension;

        // Generate signed URL for main media file (for videos, audio, and PDFs)
